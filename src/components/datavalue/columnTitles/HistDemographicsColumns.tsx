...
import { ColorInput, Radio, Group, Select, Switch, Input } from "@mantine/core";
...
export type HistDemographicsFields = {
  id: string;
  ACCT: string;
  NAME: string;
  ADATED: string | Date;
...
export const useHistDemographicsColumns = (
  data: HistDemographicsFields[],
  setData: (data: HistDemographicsFields[]) => void, // Add a setData function to update the parent state
  client: any,
  isEditAble: boolean
): MRT_ColumnDef<HistDemographicsFields>[] => {
  ...
  return [
    ...
    },
    {
      ...
      Edit: ({ row, table, cell }) => {
        const initialDate = formatDate(row.original.ADATED?.toString()); // Expecting format: YYYY-MM-DD
        ...
        return (
          <DatePickerInput
            ...
            onChange={(selectedDate) => {
              ...
              row.original.ADATED = formattedDate;
              ...
              const updatedRows = table.getRowModel().rows.map((r) =>
                r.id === cell.row.id
                  ? { ...r.original, ADATED: new Date(formattedDate) } // Update ADATED
                  : r.original
              );
        ...
        return (
          <DatePickerInput
            ...
            onChange={(selectedDate) => {
              ...
              const updatedRows = table.getRowModel().rows.map((r) =>
                r.id === cell.row.id
                  ? { ...r.original, ADATE: new Date(formattedDate) } // Update ADATE
                  : r.original
              );
    ...
    },
    {
      ...
      Edit: ({ cell, table, row }) => {
        ...
        const airOBJ = (processedOBJ || []).map((obj: { VALUE: any; VAL_DESC: any; }) => ({
          value: obj.VALUE,
          label: obj.VAL_DESC,
        }));
        return (
